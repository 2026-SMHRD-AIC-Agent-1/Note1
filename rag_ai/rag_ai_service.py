...
class FollowupQuestionDraft(BaseModel):
    question_type: Literal['직무이해', '문제해결', '협업']
    question_text: str
    question_reason: str
    evaluation_points: List[str] = Field(min_length=3, max_length=3)
...
class RagInterviewAI:
    def __init__(
        ...
        self.followup_question_llm = self.llm.with_structured_output(FollowupQuestionDraft)
...
[저장된 답변별 분석]\n{json.dumps(compact_reviews, ensure_ascii=False)}'''
    ...
    def generate_session_coaching(self, session_items: List[Dict[str, Any]]) -> Dict[str, Any]:
        """세션의 질문별 답변분석을 종합해 세션 전체 코칭과 overall_score를 만든다.

        ai_pipeline.py의 /ai/sessions/{id}/generate-coaching이 호출하는 메서드다.
        (기존 파일에는 self.session_coaching_llm만 만들어져 있고 이 메서드가
        누락되어 있어서 500 에러가 났던 부분이다.)

        overall_score 정책(MERGE_NOTES.md 기준): 답변별 (job_score+answer_score)/2를
        먼저 구하고, 그 값들의 평균을 세션 overall_score로 사용한다.
        비언어 지표는 아직 전달 점수 기준이 없으므로 참고용 코멘트로만 사용하고
        점수에는 포함하지 않는다.
        """
        if not session_items:
            raise ValueError('session_items가 비어 있습니다.')

        per_answer_scores = []
        compact_items = []
        has_nonverbal = False
        for idx, item in enumerate(session_items, 1):
            analysis = item.get('answer_analysis') or {}
            job_score = analysis.get('job_score')
            answer_score = analysis.get('answer_score')
            if job_score is None or answer_score is None:
                raise ValueError(f'{idx}번째 항목에 job_score/answer_score가 없습니다.')
            per_answer_scores.append((job_score + answer_score) / 2)
            if item.get('nonverbal_metrics'):
                has_nonverbal = True
            compact_items.append({
                'question_type': (item.get('question') or {}).get('question_type'),
                'job_evaluation': analysis.get('job_evaluation'),
                'answer_evaluation': analysis.get('answer_evaluation'),
                'strengths': analysis.get('strengths'),
                'improvements': analysis.get('improvements'),
            })

        overall_score = _round_half_up(sum(per_answer_scores) / len(per_answer_scores))

        nonverbal_note = (
            '일부 답변에 비언어 측정값이 있으나 전달 안정성 점수 기준이 아직 확정되지 않았으므로 '
            '전달 관련 코멘트는 일반적인 관찰 수준으로만 작성하고 새로운 점수를 만들지 않는다.'
            if has_nonverbal else
            '비언어 측정값이 없으므로 delivery_summary는 전달 방식에 대한 일반적인 조언 수준으로만 작성한다.'
        )

        prompt = f'''너는 모의면접 세션 종료 후 종합 코칭을 만드는 AI다.
아래는 이 세션에서 이미 평가를 마친 질문별 결과다. 점수를 다시 매기지 않고 저장된 평가만 근거로 사용한다.
저장된 strengths/improvements에 없는 새로운 평가기준을 임의로 추가하지 않는다.
합격 가능성, 기업 내부 채점기준, 성격·감정·자신감은 추측하지 않는다.
{nonverbal_note}

[기업] {self.company}
[직무] {self.job}
[내용 종합 점수] {overall_score}
[질문별 평가]
{json.dumps(compact_items, ensure_ascii=False)}

content_summary에는 내용 면에서 잘한 점과 아쉬운 점을 종합해서 작성하고,
delivery_summary에는 전달 방식(말하기 습관 등) 관련 코멘트를 작성하고,
priority_focus에는 다음에 가장 먼저 고칠 점 하나를,
next_practice_goal에는 다음 연습에서 시도해볼 구체적인 목표를 작성하라.'''
    ...
    def generate_followup_question(
        ...
        interview_type: str = 'AISK',
    ) -> Dict[str, Any]:
        """1회차 코칭(우선 개선점·다음 목표)을 반영한 다음 회차 연습 질문을 1개 생성한다.

        DEEP_INTERVIEW의 다음 회차용(1회차와 동일하게 첫 질문 1개로 시작). AISK의
        다음 회차는 generate_followup_questions()(복수형, 3개)를 사용한다.

        company/job 인자는 호출부 시그니처 호환을 위해 받지만, 이 서비스는
        생성 시점에 고정된 self.company/self.job 자료로만 RAG 색인이 되어 있어
        실제 검색은 항상 self.company/self.job 기준으로 수행한다. 세션마다
        다른 기업/직무로 다음 회차를 만들려면 그 조합으로 RAG 인덱스를 다시
        만들어야 한다 (현재 MVP 범위 밖).
        """
        if interview_type not in INTERVIEW_TYPES:
            raise ValueError(f'지원하지 않는 면접유형입니다: {interview_type}')

        coaching = previous_session.get('final_coaching', {}) or {}
        priority_focus = coaching.get('priority_focus', '') or ''
        next_practice_goal = coaching.get('next_practice_goal', '') or ''

        previous_texts = []
        for item in previous_session.get('questions', []) or []:
            q = item.get('question', {}) if isinstance(item, dict) else {}
            text = q.get('question_text')
            if text:
                previous_texts.append(text)
        previous_text_block = '\n'.join(f'- {t}' for t in previous_texts) or '없음'

        sources = self.retrieve_question_sources(
            interview_type, query_hint=f'{priority_focus} {next_practice_goal}'
        )
        prompt = f'''너는 기업·직무 맞춤형 반복연습 질문 생성 AI다.
지원자가 이전 회차 면접을 마쳤고, 아래는 그 코칭 결과다.
[이전 회차 우선 개선점] {priority_focus}
[이전 회차 다음 연습 목표] {next_practice_goal}
[이전 회차에 이미 나온 질문들 - 반복 금지]
{previous_text_block}

아래 RAG 자료만 근거로, 위 우선 개선점과 다음 연습 목표를 확인할 수 있는 새 질문을 정확히 1개 생성하라.
이전 회차 질문과 주제·표현이 겹치지 않게 한다.
evaluation_points는 정확히 3개다. 현재 선택 직무의 업무와 역량만 사용한다.
기업 내부 평가기준과 합격 가능성은 추측하지 않는다. 사용한 근거는 SOURCE 번호만 선택한다.
practice_reason에는 이전 회차의 어떤 개선점을 반영해 이 질문을 냈는지 한 문장으로 작성하라.
[RAG 자료]
{self._context(sources)}'''
        draft = self.followup_question_llm.invoke(prompt)
        finalized = self._finalize_questions([draft], sources)[0]
        finalized['practice_reason'] = draft.practice_reason
        return finalized
    ...
    def generate_followup_questions(
        ...
        """다음 회차 연습 질문을 1회차와 동일한 개수(직무이해/문제해결/협업 3개)로 생성한다.

        generate_followup_question()의 복수형 버전. AISK의 다음 회차에서
        1개만 나오던 것을 1회차와 같은 3개로 맞추기 위해 추가했다.
        """
        if interview_type not in INTERVIEW_TYPES:
            raise ValueError(f'지원하지 않는 면접유형입니다: {interview_type}')
...
아래 RAG 자료만 근거로, 위 우선 개선점과 다음 연습 목표를 확인할 수 있는 새 질문을 정확히 3개
생성하라: 직무이해 1개, 문제해결 1개, 협업 1개.
이전 회차 질문과 주제·표현이 겹치지 않게 한다.
각 질문의 evaluation_points는 정확히 3개다. 현재 선택 직무의 업무와 역량만 사용한다.
기업 내부 평가기준과 합격 가능성은 추측하지 않는다. 사용한 근거는 SOURCE 번호만 선택한다.
[RAG 자료]
{self._context(sources)}'''
        response = self.question_set_llm.invoke(prompt)
        order = {'직무이해': 0, '문제해결': 1, '협업': 2}
        drafts = sorted(response.questions, key=lambda x: order.get(x.question_type, 99))[:3]
        finalized = self._finalize_questions(drafts, sources)
        reason = (
            f"이전 회차 우선 개선점({priority_focus})을 반영해 생성됨"
            if priority_focus else "이전 회차 코칭을 반영해 생성됨"
        )
        for item in finalized:
            item['practice_reason'] = reason
        return finalized
