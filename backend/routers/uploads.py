...
4. 로컬 비언어 AI -> 실제 음성/영상 측정 + delivery_profile 생성
   (세션에 통과한 캘리브레이션이 있으면 그 baseline을 재사용)
5. 안전한 타임라인 이벤트 저장
6. 오디오 측정이 유효할 때만 RAG·언어 AI 내용평가
7. delivery_scoring으로 전달 안정성 0~100 점수 계산 후 DELIVERY_ANALYSES에 저장
   (5개 항목 중 하나라도 측정 불가면 부분 평균 없이 measurement_unavailable)
"""
...
from nonverbal_service import analyze_answer_delivery, build_calibration_object
from ai_service_client import get_ai_service, is_ready as ai_is_ready
from path_config import resolve_project_path
from integration_models import CalibrationProfiles, DeliveryAnalyses
from integration_contract_v1 import CALIBRATION_STATUS_PASSED
from delivery_scoring import compute_delivery_score
...
async def submit_answer(
    ...
    # 세션에 통과한 캘리브레이션이 있으면 그 baseline을 답변 분석에 그대로 재사용한다.
    # (통합 규격 V1 원칙: 면접 중간에 다시 캘리브레이션하지 않는다.)
    calibration_row = session.exec(
        select(CalibrationProfiles).where(CalibrationProfiles.session_id == session_id)
    ).first()
    calibration_obj = None
    if calibration_row and calibration_row.calibration_status == CALIBRATION_STATUS_PASSED:
        calibration_obj = build_calibration_object(
            calibration_row.visual_baseline, calibration_row.audio_baseline
        )
    ...
    # 전달 안정성 최종 점수 계산 + DELIVERY_ANALYSES 저장 (측정 실패 항목이 있으면
    # 부분 평균 대신 measurement_unavailable로 저장하고 delivery_score는 None).
    delivery_score_result = compute_delivery_score(delivery_profile if delivery_analysis.get("status") == "available" else None)
    delivery_analysis["delivery_score_result"] = delivery_score_result

    if delivery_analysis.get("status") == "available":
        existing_delivery = session.exec(
            select(DeliveryAnalyses).where(DeliveryAnalyses.answer_id == answer.answer_id)
        ).first()
        delivery_row = existing_delivery or DeliveryAnalyses(answer_id=answer.answer_id)
        delivery_row.status = delivery_score_result["status"]
        delivery_row.delivery_profile = delivery_profile
        delivery_row.delivery_score = delivery_score_result["delivery_score"]
