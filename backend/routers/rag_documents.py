...
통합 RAG 서비스의 SOURCE_RULES 기준으로 각 파일을
...
def sync_rag_documents_from_files(session: Session) -> SyncResult:
    ...
    data_dir = resolve_project_path(os.getenv("RAG_DATA_DIR"), "rag_data")
    if not data_dir.exists():
        raise HTTPException(status_code=404, detail=f"RAG_DATA_DIR을 찾을 수 없습니다: {data_dir}")
    # ZIP 병합 과정에서 rag_data/rag_data가 생겨도 실제 문서 디렉터리를 자동 선택합니다.
    nested = data_dir / "rag_data"
    if nested.is_dir() and not any(p.is_file() for p in data_dir.iterdir()):
        data_dir = nested
    ...
    for path in sorted(data_dir.iterdir()):
        ...
        if suffix == ".pdf":
            ...
            content = read_text_file(str(path)).strip()
            if not content:
