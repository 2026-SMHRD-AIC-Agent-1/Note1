...
// [2026-09] 기본값을 빈 문자열(상대경로)로 바꿨습니다. vite.config.js의 proxy 설정이
// 이 상대경로 요청을 백엔드(http://127.0.0.1:8000)로 그대로 중계합니다. 그래서
// 로컬 개발이든, ngrok으로 프론트(3000)만 터널링해서 시연하든 별도 설정 없이
// 동작합니다. 백엔드를 다른 컴퓨터/다른 포트에 따로 띄운 경우에만
// VITE_API_BASE를 명시적으로 지정하세요.
...
// ---------------------------------------------------------------
// 캘리브레이션 (CALIBRATION_PROFILES) — 짧은 클립 녹화 후 업로드 방식
// ---------------------------------------------------------------
export async function submitCalibration({ session_id, videoBlob, durationSec }) {
  const form = new FormData();
  form.append("video", videoBlob, "calibration.webm");
  if (durationSec !== undefined && durationSec !== null) {
    form.append("duration_sec", String(durationSec));
  }
  return request(`/calibration/sessions/${session_id}`, { method: "POST", body: form });
}

export function getCalibration(sessionId) {
  return request(`/calibration/sessions/${sessionId}`);
}

// ---------------------------------------------------------------
// 전달 안정성 세션 요약
// ---------------------------------------------------------------
export function getDeliverySummary(sessionId) {
  return request(`/interview-sessions/${sessionId}/delivery-summary`);
}
...
// ---------------------------------------------------------------
// TTS (질문 음성 낭독) — 서버(OpenAI TTS)에서 mp3를 받아온다.
// 실패 시(키 미설정, 네트워크 오류 등) 호출한 쪽에서 브라우저 TTS로 폴백한다.
// ---------------------------------------------------------------
export async function fetchQuestionSpeech(text, voice) {
  ...
  if (!res.ok) {
    ...
    throw new ApiError(detail || `TTS 요청이 실패했습니다 (HTTP ${res.status})`, res.status, detail);
  }
