...
- 실제 FastAPI AI 실행 경로는 `rag_ai/rag_ai_service.py` 하나로 통합했습니다. `ai_service_client.py`가 이 서비스를 초기화하고 `load_and_index()`까지 1회 실행합니다.
- `RAG_DATA_DIR=./rag_data` 아래에 문서가 직접 있든 병합 ZIP의 `./rag_data/rag_data` 형태이든 자동으로 실제 문서 폴더를 찾습니다.
- TXT와 PDF를 모두 RAG 인덱스에 포함하며, `08_...` 통합 JD는 선택 직무 구간만 추출합니다.
- 답변 분석 결과의 `job_score`, `answer_score`를 `ANSWER_ANALYSES`에 저장하고, 회차 종합점수는 각 답변의 `(job_score + answer_score) / 2` 평균으로 계산하여 `FINAL_COACHINGS.overall_score`에 저장합니다.
- 1회차/2회차 비교는 비언어 데이터가 없어도 `overall_score` 변화량을 표시합니다. 비언어 데이터가 들어오면 기존 비언어 지표 비교도 함께 동작합니다.
- `users` 조회 응답에서는 `password_hash`를 제외합니다. 인증/로그인 자체는 아직 MVP 범위입니다.
- 비언어 분석 자체는 이번 보완 범위에서 구현하지 않았습니다.


## MVP 환경 보완 사항
- `RAG_DATA_DIR`, `UPLOAD_DIR`, 기본 SQLite DB 경로는 프로젝트 폴더 기준으로 안정적으로 해석합니다.
- 한국어 RAG TXT는 UTF-8/UTF-8-SIG/CP949/EUC-KR을 순차 지원합니다.
- 기존 SQLite에 추가된 nullable/default 컬럼은 앱 시작 시 MVP 범위에서 자동 보완합니다.
- 음성/영상 업로드는 chunk 단위로 저장하고 최대 용량을 메모리에 올리기 전에 제한합니다.
