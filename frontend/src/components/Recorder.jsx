  ...
  async function enableCamera() {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        // 프레임레이트를 브라우저/드라이버가 알아서 정하게 두면 조명이 조금만
        // 어두워도 5~10fps까지 뚝 떨어져서 버벅여 보인다. 최소 24, 가능하면
        // 30fps를 명시적으로 요청한다(카메라가 지원 못 하면 자동으로 가까운
        // 값으로 낮아질 뿐, 에러는 나지 않는다).
  ...
  function startRecording() {
    ...
    try {
      const videoRecorder = new MediaRecorder(
        stream,
        videoMime ? { mimeType: videoMime, videoBitsPerSecond: 2_500_000 } : undefined
      );
  ...
  return (
    <div>
      ...
      <div className="btn-row">
        ...
        {permissionState === "granted" && !isRecording && !hasRecording && (
